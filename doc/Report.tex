...
Zur Repräsentation von Strecken dient Klasse \texttt{Line}, welche sowohl einen Start- als auch einen Endpunkt speichert. Die Methode 
\texttt{eval(self, x\_coord)} ermittelt für die übergebene $x$-Koordinate die zugehörige $y$-Koordinate auf der durch die Strecke induzierte Linie.\\
...
Innerhalb der Schleife wird zunächst ermittelt, welche der bereits in die Trapezzerlegung eingefügten Trapeze geschnitten werden. Handelt es sich dabei bloß um ein Trapez, ist eine andere Behandlung erforderlich als wenn es sich um mehrere Trapeze handelt. Abbildung \ref{fig:one_trap_inside} zeigt den Fall, dass die neu eingefügt Strecke $\overline{pq}$ innerhalb exakt eines Trapezes liegt.
\begin{figure}[h!]
	\centering
	\begin{tikzpicture}[scale=0.8]
		\node (1) at (-1,1.3) {\bul};
